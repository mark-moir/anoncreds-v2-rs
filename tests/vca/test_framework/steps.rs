...
pub fn step_create_accumulators_for_issuer(
    ...
    let create_accumulators = tuf::create_accumulators(vca_api.create_accumulator_data.clone());
    Arc::new(move |ts| {
        pprintln(
            "step",
...
pub fn step_sign_credential(
    ...
    let sign = vca_api.sign.clone();
    let get_range_proof_max_value = vca_api.get_range_proof_max_value.clone();
    Arc::new(move |ts| {
...
pub fn step_accumulator_add_remove(
    ...
    let convert_to_accum_elt = move |dv: &api::DataValue| -> VCAResult<api::AccumulatorElement> {
    ...
    Arc::new(move |ts| {
        ...
        let adds_with_acc_elts = adds
            .clone()
            .into_iter()
            .map(|(k, dv)| -> VCAResult<_> { Ok((k, convert_to_accum_elt(&dv)?)) })
            .collect::<VCAResult<HashMap<HolderLabel, api::AccumulatorElement>>>()?;
        let rems_with_acc_elts = removes
            .clone()
            .into_iter()
            .map(|dv| convert_to_accum_elt(&dv))
            .collect::<VCAResult<Vec<api::AccumulatorElement>>>()?;
...
pub fn step_in_range(
    ...
    let create_range_proof_proving_key = vca_api.create_range_proof_proving_key.clone();
    let max_range_value = (vca_api.get_range_proof_max_value)();
    Arc::new(move |ts| {
        // the max_off is Some(n), step_in_range modifies the upper bound of the range, making it the
        // maximum value supported by the underlying VcaApi plus n, and throwing an error in
        // case that addition overflows.  A non-zero offset is only used for testing that the
        // underlying VcaApi accurately reports the *maximum* value that can be proved to be
...
pub fn step_create_and_verify_proof(
    ...
    let create_proof = vca_api.create_proof.clone();
    let verify_proof = vca_api.verify_proof.clone();
    Arc::new(move |ts| {
        ...
        // Uncomment and run with: cargo test vca::r#impl::test_framework::proof_system::direct::tests::test_001_reveal_metadata_from_two_credentials_equality_for_two_attributes -- --nocapture
