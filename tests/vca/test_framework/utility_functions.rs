...
// The utilities below use functions provided by the vca API to implement
...
// vca API.  For example, the create_accumulators utility is used by
// step_create_accumulators_for_issuer to call the create_accumulator_data
// function provided by the vca API once for each attribute of type
...
pub type AccumulatorAddRemoveWithMap = Arc<
    dyn Fn(
            ...
            &[AccumulatorElement],
        ) -> VCAResult<AccumulatorAddRemoveWithMapResponse>
        + Send
        + Sync,
>;

pub type GetAccumulatorFromMap = Arc<
    dyn Fn(&AccumsForSigner, CredAttrIndex, AccumulatorBatchSeqNo) -> VCAResult<Accumulator>
        + Send
        + Sync,
>;
