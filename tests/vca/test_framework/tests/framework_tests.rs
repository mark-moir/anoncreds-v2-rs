...
macro_rules! testing_framework_test {
    ($crypto_interface: ident) => {
        mod testing_framework {
            ...
            use $crate::vca::test_framework::tests::framework_tests::*;
            use $crate::vca::zkp_functionality_tests::test_definitions::*;
            // -----------------------------------------------------------------
            ...
            $crate::test_framework_test!{ &implement_vca_api_using(&$crypto_interface), hashmap!() }
        }
    };
}
...
macro_rules! test_framework_test {
    ($vca_api: expr, $lib_spec: expr) => {
        $crate::initial_test_via_new_framework_test! { $vca_api, $lib_spec }
        $crate::pok_and_reveal_metadata_test_detailed_cond_test! { $vca_api, $lib_spec }
        $crate::pok_and_reveal_metadata_test_test! { $vca_api}
        $crate::pok_and_reveal_metadata_and_eqs_test! { $vca_api}
        ...
        mod pok_and_reveal_metadata_test_detailed_cond {
            ...
            lazy_static! {
                static ref TS: tf::TestState =
                    tf::start_test($vca_api, POK_AND_REVEAL_METADATA.to_owned()).unwrap();
            }
    ...
    ($vca_api: expr, $lib_spec: expr) => {
        mod initial_test_framework_test {
            ...
            fn rejects_duplicate_issuer_labels() {
                assert!(
                    tf::start_test($vca_api, vec![
                        CREATE_D_ISSUER.to_owned()
                    ])
                        .and_then(|mut ts| tf::extend_test($vca_api, vec![CREATE_D_ISSUER.to_owned()], &mut ts))
                        .is_err_containing("step_create_issuer; Duplicate issuer label; DMV"))}

            #[test]
            fn adds_issuers_signer_data() {
                let ts = tf::start_test($vca_api, vec![
                    CREATE_D_ISSUER.to_owned(),
            ...
            fn adds_issuers_public_data_to_sparms() {
                let mut ts = tf::start_test($vca_api, vec![CREATE_D_ISSUER.to_owned()]).unwrap();
                tf::extend_test($vca_api, vec![CREATE_S_ISSUER.to_owned()], &mut ts).unwrap();
                assert_eq!(
            ...
            fn adds_authority_public_data_to_sparms() {
                let ts = tf::start_test($vca_api, vec![CREATE_POLICE_AUTHORITY.to_owned()]).unwrap();
                assert_eq!(
                    ts.sparms.into_keys().collect::<Vec<_>>().into_iter().collect::<BTreeSet<_>>() ,
            ...
            #[test]
            fn adds_decrypt_req() {
                let mut ts = tf::start_test($vca_api, vec![CREATE_POLICE_AUTHORITY.to_owned()]).unwrap();
                tf::extend_test($vca_api, vec![CREATE_D_ISSUER_WITH_VE.to_owned(),sign_d_cred(td::HOLDER_1.to_owned())], &mut ts).unwrap();
                tf::extend_test($vca_api, vec![ENCRYPT_FOR_POLICE_AUTHORITY.to_owned(),], &mut ts).unwrap();
                tf::extend_test($vca_api, vec![DECRYPT_FOR_POLICE_AUTHORITY.to_owned(),], &mut ts).unwrap();
