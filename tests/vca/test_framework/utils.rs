...
pub fn start_test(vca_api: &api::VcaApi, t_seq: TestSequence) -> VCAResult<TestState> {
    ...
    extend_test(vca_api, t_seq, &mut test_state)?;
    Ok(test_state)
}
    ...
    for t_step in t_seq {
        match t_step {
            TestStep::CreateIssuer(i_lbl, schema, blind_attr_idxs, proof_mode) => {
                (*step_create_issuer(vca_api, i_lbl, schema, blind_attr_idxs, proof_mode))(t_st)?
            }
            ...
            TestStep::Reveal(h_lbl, i_lbl, idxs) => {
                (*step_reveal(vca_api, h_lbl, i_lbl, idxs))(t_st)?
            }
            TestStep::InRange(h_lbl, i_lbl, idx, min_v, max_v, max_off) => {
                (*step_in_range(vca_api, h_lbl, i_lbl, idx, min_v, max_v, max_off))(t_st)?
            }
            TestStep::InAccum(h_lbl, i_lbl, idxs, sn) => {
                (*step_in_accum(vca_api, h_lbl, i_lbl, idxs, sn))(t_st)?
            }
