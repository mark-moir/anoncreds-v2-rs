...
use credx::vca::Error;
use credx::vca::UnexpectedError;
use credx::vca::r#impl::catch_unwind_util::*;
// ------------------------------------------------------------------------------
...
// This function models the behavior of VCA functions that
// - return a VCAResult (i.e., either Ok or Err).
// - or panics in different ways (for example, an underlying library panics)

fn foo(i : i32) -> VCAResult<i32>
{
    match i
    {
        -4 => {
...
fn bar(i : i32, f : f32, s : &str) -> VCAResult<(i32, f32, &str)> {  Ok((i, f, s)) }

#[test]
fn catch_unwind_test() -> VCAResult<()> {
    // the line number is available, but not tested to make the test more stable
    const LOCATION_STRING : &str =
        "fn: \"vca_test::vca::impl::catch_unwind_test::foo\", file: \"./tests/vca/impl/catch_unwind_test.rs\", line:";
