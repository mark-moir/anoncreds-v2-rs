...
use credx::vca::r#impl::util::ic_semi;
use credx::vca::Error;

pub fn missing_case_insensitive(msg: &str, t: String, l: Vec<String>) -> Option<Error> {
    use missing_case_insensitive_support::desensitize;
    let missing: Vec<_> = l
        .iter()
        .filter(|x| !desensitize(t.clone()).contains(&desensitize(x.to_string())))
        .collect();
    if missing.is_empty() {
        None
    } else {
        Some(Error::General(ic_semi(&str_vec_from!(
            msg,
            t,
...
pub fn missing_case_insensitive_const(msg: &str, t: String, l: &[&str]) -> Option<Error> {
