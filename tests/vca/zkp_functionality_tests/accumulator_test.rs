...
lazy_static! {
    static ref AC2C_BBS_API: VcaApi =
        credx::vca::api_utils::implement_vca_api_using(&CRYPTO_INTERFACE_AC2C_BBS);
    static ref AC2C_PS_API: VcaApi =
        credx::vca::api_utils::implement_vca_api_using(&CRYPTO_INTERFACE_AC2C_PS);
    static ref DNC_API: VcaApi =
...
macro_rules! run_vca_membership_test_with {
    ($what:ident, $api:expr, $test_get_witness:expr) => {
        paste::item! {
            #[test]
            #[allow(non_snake_case)]
            fn [< vca_membership_test_ $what _ $test_get_witness>]() {
                test_vca_membership($api.clone(), $test_get_witness)
            }
        }
    };
}
...
#[derive(Eq, PartialEq)]
enum GetOrUpdateWitness {
    Get,
    Update,
}
use GetOrUpdateWitness::{Get, Update};

run_vca_membership_test_with!(ac2c_bbs_api, AC2C_BBS_API, Update);
run_vca_membership_test_with!(ac2c_bbs_api, AC2C_BBS_API, Get);
run_vca_membership_test_with!(ac2c_ps_api, AC2C_PS_API, Update);
run_vca_membership_test_with!(ac2c_ps_api, AC2C_PS_API, Get);
run_vca_membership_test_with!(dnc_api, DNC_API, Update);
run_vca_membership_test_with!(dnc_api, DNC_API, Get);

fn test_vca_membership(api: VcaApi, test_get_witness: GetOrUpdateWitness) {
    setup();
    let nonce = "asd1234098(*#$#$(*U".into();
    let res = vca_membership(api, Some(nonce), test_get_witness);
...
fn vca_membership(
    ...
    let sd = create_signer_data(0, &[ClaimType::CTAccumulatorMember], &[], ProofMode::Strict)?;
    ...
    } = accumulator_add_remove(&acc_data, &acc_val_1, &add, &[])?;
    ...
    let mem_witness_2 = if test_get_witness == Get {
        &mem_witness_2b
    } else {
        mem_witness_2a
    };
    ...
    } = accumulator_add_remove(&acc_data, &acc_val_2, &added, &[])?;
    ...
    match v_3  {
        Ok(_) => {
            return Err(vca::Error::General("v_3 verify should have failed".into()))
        },
        Err(Error::General(e))
            // DNC
        ...
        Err(_e) => {
            return Err(vca::Error::General("v_3 verify returned unexpected error".into()))
        }
    }?;
    ...
    let mem_witness_3 = if test_get_witness == Get {
    ...
    } = accumulator_add_remove(&acc_data, &acc_val_3, &empty_added, &removed)?;
    ...
    let mem_witness_4: AccumulatorMembershipWitness = match mem_witness_4_option {
        ...
        Err(_) => return Err(vca::Error::General("mem_witness_4 update returned unexpected error".into())),
    ...
    let sard = mk_sard(CRED_LABEL.into(), signature, &values, mem_witness_4.clone());
    ...
    match v_4 {
        Ok(_) => {
            println!("OK");
            return Err(vca::Error::General("v_4 verify should have failed".into()));
        }
        Err(Error::CryptoLibraryError(e))
            if e.contains(&"the presentation proof failed, the expected challenge".to_string()) =>
        {
        ...
        Err(_e) => {
            println!("UNEXPECTED");
            return Err(vca::Error::General(
                "v_4 verify returned unexpected error".into(),
            ));
        }
    }?;
