...
fn field_element_as_bytes(x: &mut Vec<u8>) -> VCAResult<Fr> {
...
fn fr_from_uint8_array(bytes: &mut Vec<u8>) -> VCAResult<Fr> {
...
fn fr_to_uint8_array(fr: Fr) -> VCAResult<Vec<u8>> {
    let mut bytes = vec![];
...
pub fn text_to_field_element(t: String) -> VCAResult<Fr> {
    let bs = &mut t.as_bytes().to_vec();
    text_to_reversibly_encoded_bytestring(bs)?;
    field_element_as_bytes(bs)
}

fn text_to_reversibly_encoded_bytestring(t: &mut Vec<u8>) -> VCAResult<&mut Vec<u8>> {
    let orig_len = t.len();
    if (orig_len > MAX_ENCODED_LENGTH) {
        ...
        for i in orig_len..MAX_ENCODED_LENGTH {
            t.push(0);
        }
        Ok(t)
    }
...
pub fn field_element_to_string(fr: Fr) -> VCAResult<String> {
