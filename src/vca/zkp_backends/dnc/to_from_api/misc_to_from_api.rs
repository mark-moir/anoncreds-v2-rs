...
impl VcaTryFrom<(ImplSignature, Vec<api::DataValue>, AccumWitnesses)>
    for api::SignatureAndRelatedData
{
    fn vca_try_from(
...
impl VcaTryFrom<&api::SignatureAndRelatedData>
    for (ImplSignature, Vec<api::DataValue>, AccumWitnesses)
{
    fn vca_try_from(
        x: &api::SignatureAndRelatedData,
    ) -> VCAResult<(ImplSignature, Vec<api::DataValue>, AccumWitnesses)> {
        let api::SignatureAndRelatedData {
            signature,
            values,
            accumulator_witnesses,
        } = x;
        Ok((
            from_api(signature)?,
            values.to_vec(),
            from_api(accumulator_witnesses)?,
        ))
    }
}
