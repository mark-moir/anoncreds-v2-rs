...
use crate::vca::r#impl::general::presentation_request_setup::is_cred_resolved;
use crate::vca::r#impl::general::presentation_request_setup::POK_OF_SIGNATURE_APPLIES_TO_ALL_ATTRIBUTES;
...
use crate::vca::zkp_backends::dnc::reversible_encoding::*;
use crate::vca::zkp_backends::dnc::signer::*;
use crate::vca::zkp_backends::dnc::types::*;
use crate::vca::{convert_to_crypto_library_error, Error, VCAResult};
// ------------------------------------------------------------------------------
...
pub fn specific_prover_dnc() -> SpecificProver {
    Arc::new(|prf_instrs, eqs, sigs_and_related_data, nonce| {
        let (
            WarningsAndResult {
                warnings,
                result: (proof_spec, maybe_witnesses),
            },
            _,
        ...
        if let Some(witnesses) = maybe_witnesses {
            ...
            Err(Error::General(
                "specific_prover_dnc; Prover must provide SignaturesAndRelatedData".to_string(),
            ))
        }
    })
}
...
fn get_decryption_responses(
    ...
    decr_reqs: &HashMap<
        CredentialLabel,
        HashMap<CredAttrIndex, HashMap<SharedParamKey, DecryptRequest>>,
    >,
) -> VCAResult<
...
fn get_decryption_response_for_cred(
    ...
    let (ct, _) = prf
        .get_saver_ciphertext_and_proof(*s_idx)
    ...
    let (decr_val, nu) = ct
        .decrypt_given_groth16_vk(&sk, dk, &snark_proving_key.pk.vk, *chunk_bit_size)
        .map_err(|e| Error::General(format!("DNC decrypt_given_groth16_vk {:?}", e)))?;
    Ok(DecryptResponse {
        value: field_element_to_string(decr_val)?,
...
fn verify_decryption_response(
    ...
    let dk_api = lookup_throw_if_absent(
        &auth_label,
        auth_dks,
        Error::General,
        &[
...
fn add_statement_and_maybe_witness(
    ...
    pig: ProofInstructionGeneral<SupportedRequirement>,
) -> VCAResult<()> {
    let ProofInstructionGeneral {
        cred_label,
        attr_idx_general,
        related_pi_idx,
        requirement: discl_general,
    } = pig;
    match discl_general {
        SupportedRequirement::PoKofSignature(
            sig_pars,
            pk,
            schema,
            revealed_idxs_and_frs_for_cred,
        ) => {
            ...
            if let Some(sigs) = sigs_mb {
                stmts.add(
                    bbs_plus::PoKBBSSignatureG1Prover::new_statement_from_params(
                        *sig_pars, revealed,
                    ),
                );
                ...
                let frs = generate_frs_from_vals_and_cts(
                    vals,
                    &schema,
                    "add_statement_and_maybe_witness",
                )?;
                add_pok_witness_new(witnesses, ((sig, frs), revealed_idxs_and_frs_for_cred));
            } else {
                stmts.add(
                    bbs_plus::PoKBBSSignatureG1Verifier::new_statement_from_params(
                        *sig_pars, *pk, revealed,
                    ),
                );
            }
            Ok(())
        ...
        SupportedRequirement::InAccumProof(acc_pars, acc_pk, mpk, acc_val, _seq_num) => {
            stmts.add(VBAccumulatorMembership::new_statement_from_params(
                *acc_pars,
                *acc_pk,
                *mpk,
                *acc_val.value(),
            ...
            if let Some(sigs) = sigs_mb {
                ...
                let fr = generate_fr_from_val_and_ct((
        ...
        SupportedRequirement::RangeProof(snark_pk, min, max) => {
            ...
            let rng_prf_stmt = match sigs_mb {
                None => BoundCheckVerifierStmt::new_statement_from_params(min, max, snark_pk.vk)
                    .map_err(|e| {
                        Error::General(format!(
                            "DNC add_statement_and_maybe_witness RangeProof None {:?}",
                            e
                        ))
                    })?,
                (Some(_)) => BoundCheckProverStmt::new_statement_from_params(min, max, *snark_pk)
                    .map_err(|e| {
                    Error::General(format!(
                        "DNC add_statement_and_maybe_witness RangeProof Some {:?}",
                        e
                    ))
                })?,
            };
            ...
            if let Some(sigs) = sigs_mb {
                ...
                if (range_proof_cheat_offset() != 0) {
                    println!(
                        "WARNING: Prover is offsetting value for range proof by {}",
                        range_proof_cheat_offset()
                    );
                }
                witnesses.add(Witness::BoundCheckLegoGroth16(generate_fr_from_val_and_ct(
                    (&ClaimType::CTInt, &val_p),
                )?));
            }
            Ok(())
        }

        SupportedRequirement::EncryptedForProof(auth_pub_label, auth_pub_data) => {
            let (AuthorityPublicSetupData {
                chunk_bit_size,
                chunked_comm_gens,
                enc_gens,
                encryption_key,
            ...
            let enc_for_stmt = match sigs_mb {
                ...
                .map_err(|e| {
                    Error::General(format!(
                        "DNC add_statement_and_maybe_witness EncryptedForProof None {:?}",
                        e
                    ))
                })?,
                ...
                .map_err(|e| {
                    Error::General(format!(
                        "DNC add_statement_and_maybe_witness EncryptedForProof Some {:?}",
                        e
                    ))
                })?,
            };
            ...
            if let Some(sigs) = sigs_mb {
                ...
                let val = get_data_value(vals, attr_idx_general)?;
                witnesses.add(Witness::Saver(generate_fr_from_val_and_ct((
                    &ClaimType::CTEncryptableText,
                    val,
                ))?));
            }
            let ix = (cred_label, attr_idx_general, auth_pub_label);
...
) -> VCAResult<&'a (ImplSignature, Vec<DataValue>, AccumWitnesses)> {
    lookup_throw_if_absent(
        c_lbl,
        sigs,
        Error::General,
        &[
            "add_statement_and_maybe_witness".to_string(),
            "getSigValsWits".to_string(),
        ],
    )
}

fn get_data_value(vals: &[DataValue], a_idx: CredAttrIndex) -> VCAResult<&DataValue> {
...
) -> VCAResult<&'a MembershipWitness<G1>> {
    lookup_throw_if_absent(
        a_idx,
        wits,
        Error::General,
...
fn transform_resolved_disclosure(
    prf_instr: &ProofInstructionGeneral<ResolvedRequirement>,
) -> VCAResult<Validation<ProofInstructionGeneral<SupportedRequirement>>> {
    let ProofInstructionGeneral {
        cred_label,
        attr_idx_general,
        related_pi_idx,
    ...
    match discl_general {
        ResolvedRequirement::CredentialResolvedWrapper(CredentialResolved {
            ...
            Ok(success(ProofInstructionGeneral {
                ...
                requirement: SupportedRequirement::PoKofSignature(
                    Box::new(sig_pars),
                    Box::new(pk),
        ...
        ResolvedRequirement::EncryptedForResolvedWrapper(EncryptedForResolved {
            ...
            Ok(success(ProofInstructionGeneral {
                ...
                attr_idx_general: *attr_idx_general,
                related_pi_idx: *related_pi_idx,
                requirement: SupportedRequirement::EncryptedForProof(
                    auth_pub_label.to_string(),
                    Box::new(apd),
                ),
            }))
        }
...
fn optimize_supported_disclosures(
    x: Vec<ProofInstructionGeneral<SupportedRequirement>>,
) -> Vec<ProofInstructionGeneral<SupportedRequirement>> {
    x
}

fn get_supported_disclosures(
    prf_mode: bool,
    prf_instrs: &[ProofInstructionGeneral<ResolvedRequirement>],
) -> VCAResult<WarningsAndResult<Vec<ProofInstructionGeneral<SupportedRequirement>>>> {
    let WarningsAndResult { warnings, result } = transform_instructions(prf_mode, prf_instrs)?;
    Ok(WarningsAndResult {
        warnings,
        result: optimize_supported_disclosures(result),
    })
}
...
fn proof_spec_from(
    prf_mode: bool,
    prf_instrs: &Vec<ProofInstructionGeneral<ResolvedRequirement>>,
    eqs: &EqualityReqs,
    sigs_and_related_data_mb: Option<&HashMap<CredentialLabel, SignatureAndRelatedData>>,
) -> VCAResult<(
    WarningsAndResult<(ProofSpec<Bls12_381>, Option<Witnesses<Bls12_381>>)>,
    DecryptionLookups,
)> {
    let WarningsAndResult {
        warnings,
        result: dnc_instrs_opt,
    ...
    let (statements, witnesses, decr_lkups) =
        create_statements_and_maybe_witnesses(dnc_instrs_opt, &sards_mb)?;
...
fn create_statements_and_maybe_witnesses(
    l: Vec<ProofInstructionGeneral<SupportedRequirement>>,
    sigs_mb: &Option<
        (HashMap<
            CredentialLabel,
            (
...
fn add_implicit_equality_if_needed(
    ...
    resolved_prf_instr: &ProofInstructionGeneral<ResolvedRequirement>,
) {
    let ProofInstructionGeneral {
        cred_label: _,
        attr_idx_general,
...
fn create_meta_statements(
    prf_instrs: &Vec<ProofInstructionGeneral<ResolvedRequirement>>,
    eqs: &EqualityReqs,
) -> VCAResult<MetaStatements> {
    let mut meta_statements = MetaStatements::new();
    let equality_constraints = eqs
        .iter()
        .map(|x| go_eq(prf_instrs, x))
        .collect::<VCAResult<Vec<_>>>()?;
    for ecs in equality_constraints {
        let bts = ecs
            .iter()
            .map(|(ecl, ecr)| (*ecl as usize, *ecr as usize))
            .collect();
        meta_statements.add_witness_equality(EqualWitnesses(bts));
    }
...
fn lkups(
    prf_instrs: &Vec<ProofInstructionGeneral<ResolvedRequirement>>,
) -> HashMap<CredentialLabel, ProofInstructionGeneral<ResolvedRequirement>> {
...
fn add_pok_witness_new(
    ...
    let revealed_indices = revealed_idxs_and_frs
        .iter()
        .map(|(i, _)| *i)
        .collect::<Vec<u64>>();
    let (unrevealed, _revealed) = partition_frs(revealed_indices, all_frs);
    wits.add(PoKSignatureBBSG1Wit::new_as_witness(
        sig.clone(),
        unrevealed,
    ));
}
...
enum SupportedRequirement {
    PoKofSignature(
        Box<SignatureParamsG1<Bls12_381>>,
        Box<PublicKeyG2<Bls12_381>>,
        Vec<ClaimType>,
        Vec<(u64, Fr)>,
    ),
    RangeProof(Box<legogroth16::ProvingKey<Bls12_381>>, u64, u64),
    EncryptedForProof(SharedParamKey, Box<AuthorityPublicSetupData>),
    ...
    // have multiple InAccum variants in SupportedRequirement, but not necessarily multiple InAccum
