...
) -> VCAResult<HashMap<CredentialLabel,
                       HashMap<CredAttrIndex,
...
{
    let ProofInstructionGeneral { cred_label, attr_idx_general, related_pi_idx, requirement: discl_general } = pig;
    match discl_general
    {
        SupportedRequirement::PoKofSignature(sig_pars, pk, schema, revealed_idxs_and_frs_for_cred) =>
        {
...
{
    let ProofInstructionGeneral { cred_label, attr_idx_general, related_pi_idx, requirement: discl_general } = prf_instr;
    match discl_general {
        ResolvedRequirement::CredentialResolvedWrapper
        ...
        ResolvedRequirement::InRangeResolvedWrapper
        ...
        ResolvedRequirement::InAccumResolvedWrapper
        ...
        ResolvedRequirement::EncryptedForResolvedWrapper
            (EncryptedForResolved { auth_pub_label, auth_pub_data }) =>
        {
            ...
            Ok(success(ProofInstructionGeneral {
                ...
                requirement      : SupportedRequirement::EncryptedForProof
                    (auth_pub_label.to_string(), Box::new(apd))}))
        },
    }
}
...
) -> VCAResult<(WarningsAndResult<(ProofSpec::<Bls12_381>, Option<Witnesses<Bls12_381>>)>,
                DecryptionLookups)>
{
    ...
    sigs_mb   :
      &Option<(HashMap<CredentialLabel,
                       (SignatureG1::<Bls12_381>,
                        Vec<DataValue>,
                        HashMap<CredAttrIndex, MembershipWitness::<G1>>)>)>
) -> VCAResult<(Statements::<Bls12_381>, Witnesses::<Bls12_381>, DecryptionLookups)>
{
    ...
    resolved_prf_instr : &ProofInstructionGeneral<ResolvedRequirement>)
{
    let ProofInstructionGeneral
        { cred_label : _, attr_idx_general, related_pi_idx, requirement : _ } = resolved_prf_instr;
...
) -> VCAResult<MetaStatements>
{
    ...
    let equality_constraints = eqs.iter().map(|x| go_eq(prf_instrs, x)).collect::<VCAResult<Vec<_>>>()?;
