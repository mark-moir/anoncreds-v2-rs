...
pub fn generate_fr_from_val_and_ct(ct_dv: (&ClaimType, &DataValue)) -> VCAResult<Fr> {
    match ct_dv {
        ...
        (ClaimType::CTEncryptableText, DataValue::DVText(t)) => {
            Ok(text_to_field_element(t.to_string())?)
        }
        _x => Err(Error::General(format!(
            "generate_fr_from_val_and_ct, UNEXPECTED combination: {:?} {:?}",
            ct_dv.0, ct_dv.1
        ))),
    }
}
...
pub fn generate_frs_from_vals_and_cts(
    ...
    if (vals.len() != sdcts.len()) {
        Err(Error::General(format!(
            "{err_msg}, number of values and claim types unequal: {:?} {:?}",
            sdcts, vals
        )))
    } else {
        sdcts
...
fn field_elems_from_seeds(t: &String) -> VCAResult<Fr> {
