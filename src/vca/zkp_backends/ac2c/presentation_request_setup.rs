...
pub fn presentation_schema_from<S: ShortGroupSignatureScheme>(
    ...
    eqs: &EqualityReqs,
) -> VCAResult<WarningsAndResult<PresentationSchema<S>>> {
...
pub fn idx_from_attr_label(bs: &CredentialLabel) -> VCAResult<CredAttrIndex> {
...
pub fn cred_label_from_statement_id(bs: &str) -> VCAResult<CredentialLabel> {
...
#[derive(Debug)]
enum SupportedRequirement<S: ShortGroupSignatureScheme> {
...
fn transform_instructions<S: ShortGroupSignatureScheme>(
    prf_instrs: &[ProofInstructionGeneral<ResolvedRequirement>],
) -> VCAResult<WarningsAndResult<Vec<ProofInstructionGeneral<SupportedRequirement<S>>>>> {
    let (warnings0, instrs): (
        Vec<Warning>,
        Vec<ProofInstructionGeneral<SupportedRequirement<S>>>,
    ) = prf_instrs
        .iter()
...
fn generate_statements<S: ShortGroupSignatureScheme>(
    discls: &[ProofInstructionGeneral<SupportedRequirement<S>>],
) -> Vec<Statements<S>> {
    discls
        .iter()
        .map(
