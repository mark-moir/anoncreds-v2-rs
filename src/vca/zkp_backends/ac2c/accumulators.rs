...
use crate::vca::zkp_backends::ac2c::to_from_api::accumulators_to_from_api::*;
...
use crate::prelude::{
    vb20, ClaimData, CredentialBundle, Issuer, IssuerPublic, MembershipClaim, MembershipCredential,
    MembershipRegistry, MembershipSigningKey, MembershipStatement, MembershipVerificationKey,
};
...
pub fn create_accumulator_data() -> CreateAccumulatorData {
    Arc::new(|rng_seed| {
        ...
        let ad = AccumulatorData {
            accumulator_public_data: to_api(pk)?,
            accumulator_secret_data: to_api(sk)?,
        };
        Ok(CreateAccumulatorResponse {
            accumulator_data: ad,
            accumulator: to_api(acc)?,
        })
    })
}
...
pub fn create_membership_proving_key() -> CreateMembershipProvingKey {
    Arc::new(|_| {
        Ok(MembershipProvingKey(
            "not used: AC2C MembershipProvingKey".to_string(),
        ))
    })
}
...
pub fn accumulator_add_remove() -> AccumulatorAddRemove {
    Arc::new(|ad, acc, adds, rms| {
        ...
        let r = rms.iter().map(from_api).collect::<VCAResult<Vec<_>>>()?;
...
pub fn update_accumulator_witness() -> UpdateAccumulatorWitness {
    Arc::new(|witness, element, update_info| {
        ...
        let e = from_api(element)?;
        let AC2CWitnessUpdateInfo {
            ac2c_adds,
            ac2c_rms,
            ac2c_coefficients,
        } = from_api(update_info)?;
        let a = ac2c_adds
            .iter()
            .map(from_api)
            .collect::<VCAResult<Vec<_>>>()?;
