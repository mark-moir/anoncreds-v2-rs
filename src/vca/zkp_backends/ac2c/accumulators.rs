...
pub fn accumulator_add_remove() -> AccumulatorAddRemove {
    Arc::new(|ad, acc, adds, rms| {
        ...
        let a        = adds.iter().map(|(_,x)| from_api(x)).collect::<VCAResult<Vec<_>>>()?;
...
pub fn update_accumulator_witness() -> UpdateAccumulatorWitness {
    Arc::new(|witness, element, update_info| {
        ...
        let a  = ac2c_adds.iter().map(from_api).collect::<VCAResult<Vec<_>>>()?;
        let r  = ac2c_rms .iter().map(from_api).collect::<VCAResult<Vec<_>>>()?;
