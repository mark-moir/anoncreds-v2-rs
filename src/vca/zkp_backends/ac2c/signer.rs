...
use crate::vca::r#impl::util::{insert_throw_if_present, lookup_throw_if_out_of_bounds};
use crate::vca::zkp_backends::ac2c::presentation_request_setup::attr_label_for_idx;
use crate::vca::{convert_to_crypto_library_error, Error, UnexpectedError, VCAResult};
use crate::{get_location_and_backtrace_on_panic, str_vec_from};
// ------------------------------------------------------------------------------
use crate::knox::short_group_sig_core::short_group_traits::ShortGroupSignatureScheme;
use crate::prelude;
use crate::prelude::{
    ClaimData, ClaimSchema, CredentialSchema, HashedClaim, Issuer, IssuerPublic, NumberClaim,
...
static UNUSED_REVOCATION_LABEL: &str = "BOGUS KEY - SHOULD NOT CONFLICT WITH ATTRIBUTE LABELS";
...
pub fn specific_create_signer_data<S: ShortGroupSignatureScheme>() -> SpecificCreateSignerData {
    Arc::new(|_rng_seed, schema, blind_attr_idxs| {
        ...
        let blind_attrs = blind_attrs_1
            ...
            .map(|x| x.as_str())
            .collect::<Vec<&str>>();
        let cred_schema = get_location_and_backtrace_on_panic!(CredentialSchema::new(
            Some(LABEL),
            Some(DESCRIPTION),
            &blind_attrs,
            &schema_claims
...
pub fn specific_create_blind_signing_info<S: ShortGroupSignatureScheme>(
    ...
    Arc::new(|_rng_seed, spsd, schema, blind_attrs| {
        ...
        let blind_claims: BTreeMap<String, ClaimData> = blind_attrs
            .iter()
            .map(|idx_val_pair| {
                create_label_claim_pair("create_blind_signing_info, AC2C", schema, idx_val_pair)
            })
            .collect::<VCAResult<BTreeMap<_, _>>>()?;
        let (blind_credential_request, blinder) =
            BlindCredentialRequest::new(&issuer_public, &blind_claims).map_err(|e| {
...
pub fn specific_sign_with_blinded_attributes<S: ShortGroupSignatureScheme>(
    ...
    Arc::new(
        |_rng_seed, schema, non_blinded_attrs, bifs, _, signer_secret_data| {
            let mut claims: BTreeMap<String, ClaimData> = non_blinded_attrs
                .iter()
                .map(|idx_val_pair| {
                    create_label_claim_pair(
                        "sign_with_blinded_attributes, AC2C",
                        schema,
                        idx_val_pair,
                    )
                })
...
fn create_schema_claim((idx, ct): (usize, &ClaimType)) -> VCAResult<ClaimSchema> {
    let aix = attr_label_for_idx(idx as u64);
    match ct {
        ClaimType::CTText => Ok(ClaimSchema {
            claim_type: prelude::ClaimType::Hashed,
            label: aix,
            print_friendly: true,
            validators: vec![],
        }),
        ClaimType::CTEncryptableText => Ok(ClaimSchema {
            claim_type: prelude::ClaimType::Scalar,
            label: aix,
            print_friendly: true,
            validators: vec![],
        }),
        ClaimType::CTInt => Ok(ClaimSchema {
            claim_type: prelude::ClaimType::Number,
            label: aix,
            print_friendly: true,
            validators: vec![],
        }),
        ClaimType::CTAccumulatorMember => Ok(ClaimSchema {
            claim_type: prelude::ClaimType::Hashed,
            label: aix,
            print_friendly: true,
            validators: vec![],
...
// pub for test
pub fn create_schema_claims(cts: &[ClaimType]) -> VCAResult<Vec<ClaimSchema>> {
...
pub fn val_to_claim_data(ct_dv: (&ClaimType, &DataValue)) -> VCAResult<ClaimData> {
    match ct_dv {
        (ClaimType::CTText, DataValue::DVText(t)) => Ok(HashedClaim::from(t).into()),
        (ClaimType::CTEncryptableText, DataValue::DVText(t)) => Ok(ScalarClaim::encode_str(t)
            .map_err(|e| {
                Error::General(format!(
                    "val_claim_to_data: CTEncryptableText encoding error {e:?}"
                ))
            })?
            .into()),
        ...
        _x => Err(Error::General(format!(
            "val_to_claim_data, UNEXPECTED combination: {:?} {:?}",
            ct_dv.0, ct_dv.1
        ))),
    }
}

fn vals_to_claim_data(sdcts: &[ClaimType], vals: &[DataValue]) -> VCAResult<Vec<ClaimData>> {
    if sdcts.len() != vals.len() {
        Err(Error::General(format!(
            "vals_to_claim_data, number of values and claim types unequal: {:?} {:?}",
            sdcts, vals
        )))
    } else {
        sdcts
