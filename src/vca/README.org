...
#+TITLE: Overview of the Verifiable Credentials abstraction (VCA), implementation, and test framework
...
- By using the ~implement_vca_api_using~ function in [[./api_utils.rs][./api_utils.rs]] and providing an instance of
  ...
  its methods directly.  See [[connecting-zkp-libary-to-vca]] for more details.
- The API also surfaces helper functions to check "correctness proofs" on inputs
  (e.g., ~verify_signer_public_setup_data_correctness_proof~,
  ~verify_blind_signing_info_correctness_proof~,
  ~verify_signature_correctness_proof~ and
  ~verify_blind_signature_correctness_proof~).  These are currently stubs for the AC2C backend and
  partially implemented for DNC; see the sections below for intended usage.
...
as described in <<connecting-zkp-libary-to-vca>>,
...
- =SignerPublicData=
  - =SignerPublicSetupData= (opaque AC2C public setup data and a correctness proof)
  - a vector of =ClaimType= (i.e., the "schema")
  - a list of blinded attribute indices

The API also exposes ~verify_signer_public_setup_data_correctness_proof~ to validate the correctness
proof carried inside ~SignerPublicSetupData~.
...
Note that =BlindInfoForSigner= contains a blinding commitment *and* a correctness proof
(~blindSigningInfoCorrectnessProof~).  Callers can check that proof using
~verify_blind_signing_info_correctness_proof~ before invoking =sign_with_blinded_attributes=.  The
DNC backend produces and verifies this proof; the AC2C backend currently treats the proof as a
stub because the underlying library creates and verifies such a proof, which is not yet exposed
via the the correctness proof field of ~SignerPublicSetupData~.
...
The =Signature= and =BlindSignature= types now each include a companion correctness-proof field
(~signatureCorrectnessProof~ and ~blindSignatureCorrectnessProof~).  The API surfaces
~verify_signature_correctness_proof~ and ~verify_blind_signature_correctness_proof~ which callers can
use to check these.  Both backends currently return placeholder proofs, so these verifiers are
stubs for now, but the structure is in place for future concrete proofs.
