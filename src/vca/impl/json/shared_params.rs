...
pub fn lookup_one_text<'a>(k: &SharedParamKey, params: &'a SharedParams) -> VCAResult<&'a String> {
    let v = params
        .get(k)
...
pub fn lookup_one_int<'a>(k: &SharedParamKey, params: &'a SharedParams) -> VCAResult<&'a u64> {
    let v = params
        .get(k)
        .ok_or(Error::General(format!("SharedParams; missing key; {k}")))?;
    match v {
