...
pub fn create_blind_signing_info(
    ...
    Arc::new(
        move |rng_seed,
              ...
              proof_mode| {
            if proof_mode != TestBackend {
                let mut given_idxs = blinded_attrs
                    .iter()
        ...
        move |rng_seed,
            ...
            if prf_mode != TestBackend {
                let pairs = vals
                    ...
                    .enumerate()
                    .map(|(index, value)| {
                        CredAttrIndexAndDataValue::new(index as CredAttrIndex, value.clone())
                    })
                    .collect::<Vec<_>>();
...
pub fn sign_with_blinded_attributes(
    ...
    Arc::new(move |rng_seed, non_blinded_attrs, bifs, sd, proof_mode| {
        ...
        if proof_mode != TestBackend {
            let mut schema_idxs: Vec<CredAttrIndex> = (0..signer_public_schema.len())
                .map(|x| x as CredAttrIndex)
                .collect();
            let unblinded_idxs: Vec<_> = non_blinded_attrs
                .iter()
                .map(|CredAttrIndexAndDataValue { index, value: _ }| *index)
                .collect();
...
pub fn unblind_blinded_signature(
    ...
    Arc::new(
        move |schema, unblinded_attrs, blinded_sig, blinder, proof_mode| {
            ...
            spec_unblind_blinded_signature(schema, unblinded_attrs, blinded_sig, blinder)
        },
    )
}
...
fn check_attr_idxs_and_vals_for_schema(
    ...
    for CredAttrIndexAndDataValue { index, value } in pairs {
        let ct = lookup_throw_if_out_of_bounds(
            ...
            *index as usize,
            Error::General,
            &str_vec_from!("check_attr_idxs_and_vals_for_schema", "IMPOSSIBLE"),
        )?;
        match (ct, value) {
