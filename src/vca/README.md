...
below, is under the [tests/vca/test_framework](../../tests/vca/test_framework/) directory.
...
`VcaApi` type in [./api.rs](./api.rs). There are several possible ways to access it:
...
[../../tests/vca/test_framework/types.rs](../../tests/vca/test_framework/types.rs)).
...
example of a utility function that uses, but is not directly supported by `VcaApi`; see comments
in [../../tests/vca/test_framework/utility_functions.rs](../../tests/vca/test_framework/utility_functions.rs).
...
Located in [../../tests/vca/](../../tests/vca/):
...
The following diagram gives a high-level view of the VCA architecture.
...
VCA is comprised of three main parts

-   API (defined by the `VcaApi` type in [./api.rs](./api.rs))
...
VCA code resides in the [../../src/vca/](../../src/vca/) directory.

The top level directory contains:

    api.rs                                    : the main top-level VcaApi
    
    api_utils.rs                              : connects a specific CryptoInterface to the VcaApi

The directory structure for the interfaces used by `VcaApi` is:

    interfaces
        crypto_interface.rs               : function types that a specific ZKP library must implement
    
        non_primitives.rs                 : function types for functions provided by VCA
        primitives
        ...
        types.rs                          : data declarations for data used in VcaApi and CryptoInterface
...
`CRYPTO_INTERFACE_AC2C` is passed to `implement_vca_api_using` (defined in [./api_utils.rs](./api_utils.rs))
to create an instance of `VcaApi`.  Many of the primitives are directly assigned to `VcaApi` fields.
...
`VcaApi` function, which are then assigned to their associated fields.
...
To prepare for signing credentials, an Issuer uses `create_signer_data` in `VcaApi` ([./api.rs](./api.rs)).
...
-   creates an AC2C schema representation based on a list of VCA `ClaimType` and the list of blinded indices
-   creates AC2C public and secret data (that includes public/secret keys)
-   returns VCA `SignerData`
...
-   transforms human-friendly `CredentialReqs` into machine-friendly "proof instructions" and equality requirements
    ...
    -   the "proof instructions" returned by `presentation_request_setup` are of type `ProofInstructionGeneral<ResolvedRequirement>`
...
-   calls the specific ZKP library function `specific_prover`, passing the proof instructions, equality requirements, signatures, etc.
    -   Each ZKP backend we have implemented specifies its own library-specific type of "proof instruction"; we call them `ProofInstructionGeneral<SupportedRequirement>` in both cases, but this is not a requirement.
...
-   wraps that proof in a VCA opaque data type
...
converts the VCA information and data into formats used by AC2C, and then calls
...
`ResolvedRequirement::CredentialResolvedWrapper(CredentialResolved`.  It contains
...
transform the `ProofInstructionGeneral<ResolvedRequirement>` into a
`ProofInstructionGeneral<SupportedRequirement>`.
That function returns a proof instruction that contains `SupportedRequirement::SignatureAndReveal` that contains

-   an `anoncreds-v2-rs` `IssuerPublic`
-   `HashMap<CredAttrIndex, (DataValue, ClaimType)>`

`SupportedRequirement` is then given to `generate_statements` ([./zkp_backends/ac2c/presentation_request_setup.rs](./zkp_backends/ac2c/presentation_request_setup.rs)).
For `SupportedRequirement::SignatureAndReveal`, `generate_statements` creates an `anoncreds-v2-rs`
...
For range proofs,
`ResolvedRequirement::InRangeResolvedWrapper(InRangeResolved` is created. It contains
...
The AC2C implementation then transforms that `ResolvedRequirement` into
`SupportedRequirement::RangeProof` that contains

-   the `anoncreds-v2-rs` range proving key
-   the min and max values
-   the index

`SupportedRequirement::RangeProof`  is given to `generate_statements`
...
`ResolvedRequirement::EncryptedForResolvedWrapper(EncryptedForResolved)` is created. It contains

-   the API public data to be used for encryption

The AC2C implementation then transforms that `ResolvedRequirement` into
`SupportedRequirement::EncryptedFor` that contains

-   the `anoncreds-v2-rs` verifiable encryption public key

`SupportedRequirement::EncryptedFor`  is given to `generate_statements`
...
For accumulators
`ResolvedRequirement::InAccumResolvedWrapper(InAccumResolved` is created. It contains

-   the API values obtained from the keys in `InAccum`

The AC2C implementation then transforms that `ResolvedRequirement` into
`SupportedRequirement::InAccumProof` that contains

-   the `anoncreds-v2-rs` `vb20::PublicKey` for accumulators
-   the `anoncreds-v2-rs` `vb20::Accumulator` accumulator

`SupportedRequirement::InAccumProof` is given to `generate_statements`
